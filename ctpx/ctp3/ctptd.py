...
if platform.architecture()[1] == "ELF":
    from ctpx.ctp3.linux import ctptd as td
elif platform.architecture()[0] == "64bit":
    from ctpx.ctp3.win64 import ctptd as td
else:
    from ctpx.ctp3.win32 import ctptd as td
...
class CtpTd(td.CtpTd):
    ...
    def onRspUserLogin(self, RspUserLoginField, RspInfoField, requestId, final):
        ...
        if RspInfoField.errorID == 0:
            ...
            log = '交易服务登陆失败，错误码:{0}, 错误信息：{1}'.format(
                RspInfoField.errorID, RspInfoField.errorMsg)
        logger.info(log)
    ...
    def onRspUserLogout(self, UserLogoutField, RspInfoField, requestId, final):
        ...
        if RspInfoField.errorID == 0:
            ...
            log = '交易服务登出失败，错误码:[{0}], 错误信息:[{1}]'.format(
                RspInfoField.errorID, RspInfoField.errorMsg)
        logger.debug(log)
    ...
    def onRspOrderInsert(self, InputOrderField, RspInfoField, requestId, final):
        ...
        # print("onRspOrderInsert --- errorCode:{0}, errorMsg:{1}.".format(
        #     RspInfoField.errorID, RspInfoField.errorMsg))
        for listener in self._eventListeners:
    ...
    def onRspSettlementInfoConfirm(self, SettlementInfoConfirmField, RspInfoField, requestId, final):
        ...
        logger.debug("onRspSettlementInfoConfirm --- errorCode:{0}，errorMsg:{1}.".format(
            RspInfoField.errorID, RspInfoField.errorMsg))
    ...
    def onRspQryOrder(self, OrderField, RspInfoField, requestId, final):
        ...
        if RspInfoField: #这个字段竟然能能返回为空
            logger.debug("onRspQryOrder --- errorCode: {0}, errorMsg:{1}.".format(
                RspInfoField.errorID, RspInfoField.errorMsg))
    ...
    def onRspQryInvestorPosition(self, InvestorPositionField, RspInfoField, requestId, final):
        """请求查询投资者持仓响应"""
        logger.debug("onRspQryInvestorPosition -- instrumentID:{0}, brokerID:{1}, investorID:{2}, posiDirection:{3},"
        ...
        if RspInfoField: #这个字段竟然能能返回为空
            logger.debug("onRspQryInvestorPosition -- errorCode: {0}, errorMsg:{1}.".format(
                RspInfoField.errorID, RspInfoField.errorMsg))
    ...
    def onRspError(self, RspInfoField, requestId, final):
        """错误应答"""
        log = '交易服务响应错误，错误码:[{0}], 错误信息:[{1}]'.format(
            RspInfoField.errorID, RspInfoField.errorMsg)
        logger.debug(log)
    ...
    def onRspQryInvestorPositionDetail(self, InvestorPositionDetailField, RspInfoField, requestId, final):
        ...
        if InvestorPositionDetailField == None:
            return
        ...
        if RspInfoField: #这个字段竟然能能返回为空
            logger.debug("onRspQryInvestorPositionDetail -- errorCode: {0}, errorMsg:{1}.".format(
                RspInfoField.errorID, RspInfoField.errorMsg))
    ...
    def onRspOrderAction(self, InputOrderActionField, RspInfoField, requestId, final):
        """报单操作请求响应"""
        logger.debug("onRspOrderAction --- sessionId: {0}, frontID: {1}, orderRef: {2}".format(InputOrderActionField.sessionID,
                InputOrderActionField.frontID, InputOrderActionField.orderRef))
        logger.debug("onRspOrderAction --- errorCode:{0}, errorMsg:{1}.".format(
            RspInfoField.errorID, RspInfoField.errorMsg))
        for listener in self._eventListeners:
            listener.onRspOrderAction( InputOrderActionField, RspInfoField)

    def addEventListener(self, TdEventListener):
        if TdEventListener and TdEventListener not in self._eventListeners:
            self._eventListeners.add(TdEventListener)

    def removeEventListener(self, TdEventListener):
        if TdEventListener and TdEventListener in self._eventListeners:
            self._eventListeners.remove(TdEventListener)
