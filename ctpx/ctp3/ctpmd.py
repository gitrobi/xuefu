...
if platform.architecture()[1] == "ELF":
    from ctpx.ctp3.linux import ctpmd as md
elif platform.architecture()[0] == "64bit":
    from ctpx.ctp3.win64 import ctpmd as md
else:
    from ctpx.ctp3.win32 import ctpmd as md
...
class CtpMd(md.CtpMd):
    ...
    def onFrontDisconnected(self, reasonCode):
        """0x2003 收到错误报文"""
        logger.info('行情服务器连接断开，错误码是%d', reasonCode)
    ...
    def onRspUserLogin(self, RspUserLoginField, RspInfoField, requestId, final):
        ...
        if RspInfoField.errorID == 0:
            ...
            log = '行情服务登陆失败，错误码:{0}, 错误信息：{1}'.format(
    ...
    def onRspUserLogout(self, UserLogoutField, RspInfoField, requestId, final):
        ...
        if RspInfoField.errorID == 0:
            ...
            log = '行情服务登出失败，错误码:[{0}], 错误信息:[{1}]'.format(
                RspInfoField.errorID, RspInfoField.errorMsg)
        logger.info(log)

    def onRspError(self, RspInfoField, requestId, final):
        """错误应答"""
        log = '行情服务响应错误，错误码:[{0}], 错误信息:[{1}]'.format(
           RspInfoField.errorID, RspInfoField.errorMsg)
        logger.info(log)

    def onRspSubMarketData(self, SpecificInstrumentField, RspInfoField, requestId, final):
        """订阅行情应答"""
        logger.info('订阅的instrumentID：{0}，响应结果，错误码：[{1}]，错误信息：【{2}]'.format(
            SpecificInstrumentField.instrumentID, RspInfoField.errorID, RspInfoField.errorMsg))
    ...
    def onRtnDepthMarketData(self, DepthMarketDataField):
        ...
        for listener in self._eventListeners:
            listener.onRtnDepthMarketData(DepthMarketDataField)
    ...
    def addEventListener(self, mdEventListener):
        if mdEventListener and mdEventListener not in self._eventListeners:
            self._eventListeners.add(mdEventListener)

    def removeEventListener(self, mdEventListener):
        if mdEventListener and mdEventListener in self._eventListeners:
            self._eventListeners.remove(mdEventListener)
...
if __name__ == "__main__":
    ...
    ctpMd.subscribeMarketData(['SM709'])
